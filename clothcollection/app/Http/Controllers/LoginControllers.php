    ...
    public function postVendorLogin(Request $request)
    {
        ...
        foreach($results as $result)
        {
           if($result){
             ...
             return redirect('vendor-dashboard')->with('status', 'Succesful Login.');
            }
        }
        return redirect('vendor-login')->with('status', 'Email or password is incorrect.');
    }
    public function logout(Request $request)
    {
        session()->flush();
        return redirect('vendor-login')->with('status', 'Logout Successfully!');
    }
}